...
export class Engine {
    ...
    async search_engine(terme) {
        ...
        for (let i = 0; i < this.allRecipes.length; i++) {
            ...
            if (this.allRecipes[i].description.toLowerCase().includes(terme.target.value.toLowerCase())) {
                this.newAllRecipes.recipes.push(this.allRecipes[i])
            }
            for (let k = 0; k < this.allRecipes[i].ingredients.length; k++) {
                if (this.allRecipes[i].ingredients[k].ingredient.toLowerCase().includes(terme.target.value.toLowerCase())) {
                    this.newAllRecipes.recipes.push(this.allRecipes[i])
                }
            }
        }
